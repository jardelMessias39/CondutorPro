  ...
  window.dispatchEvent(evento);
    ...
    if (isLogin) {
      ...
      const { error } = await supabase
        .from('alunos')
        .insert([
          { nome, email, cpf, senha, status: 'pendente', role: 'aluno' }
        ]);

      if (error) {
        alert("Erro ao cadastrar: " + error.message);
      } else {
        alert("Matrícula realizada com sucesso! Agora faça seu login.");
        ...
        // Agora sim, mudamos para a tela de login com os campos vazios
        setIsLogin(true);
      }
    }
